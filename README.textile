...
(defmachine :door-lifecycle
  (state :locked
    {:start true
     :transitions-to [:closed]})
  (state :closed
    {:transitions-to [:open :locked]})
  (state :open 
    {:transitions-to [:closed]}))

</pre>

h1. Interactive Development with Emacs and SLIME

Start with the defmachine, add a state and any transitions.  Do this 1 at a time and eval the buffer.  Impresario's DSL will throw an exception for any missing transition predicates - alerting you to which ones are missing.
...
!https://github.com/kyleburton/impresario/raw/master/examples/simple-workflow.dot.png!
...
Introduce a second form of register-workfow that accepts a version.  Make the defult (when unspecified) "1.0.0".
